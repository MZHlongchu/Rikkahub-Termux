...
private fun ProviderConfigureOpenAI(
    ...
    CardGroup(modifier = Modifier.fillMaxWidth()) {
        ...
        item(
            headlineContent = { Text(stringResource(R.string.setting_provider_page_full_reasoning_history)) },
            supportingContent = { Text(stringResource(R.string.setting_provider_page_full_reasoning_history_warning)) },
            trailingContent = {
                val warning = stringResource(R.string.setting_provider_page_full_reasoning_history_warning)
                Switch(
                    checked = provider.sendFullReasoningHistory,
                    onCheckedChange = { enabled ->
                        onEdit(provider.copy(sendFullReasoningHistory = enabled))
                        if (enabled) {
                            toaster.show(message = warning, type = ToastType.Warning)
                        }
                    }
                )
            }
