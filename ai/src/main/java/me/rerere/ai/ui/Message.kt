...
fun List<UIMessage>.limitToolCallsByRecentMessages(keepRecentMessages: Int?): List<UIMessage> {
    if (keepRecentMessages == null || this.isEmpty()) return this

    val firstMessageToKeepTools = (this.size - keepRecentMessages.coerceAtLeast(0)).coerceAtLeast(0)
    return mapIndexedNotNull { index, message ->
        if (index >= firstMessageToKeepTools) {
            message
        } else {
            val filteredParts = message.parts.filterNot { it is UIMessagePart.Tool }
            if (filteredParts.isNotEmpty()) message.copy(parts = filteredParts) else null
        }
    }
}
